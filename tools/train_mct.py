...
def train_epoch(
    ...
    for cur_iter, (mag_map_c, mag_map_s) in enumerate(train_loader):
        # Transfer the data to the current GPU device.
        train_meter.iter_timer.reset()

        if cfg.GPU_ENABLE:
            mag_map_c = mag_map_c.to(cur_device, non_blocking=True)
            mag_map_s = mag_map_s.to(cur_device, non_blocking=True)
        #print(mag_map_r.shape)
...
def eval_epoch(val_loader, model, val_meter, cur_epoch, cfg, writer=None):
    ...
    for cur_iter, (mag_map_c, mag_map_s) in enumerate(val_loader):
        ...
        val_meter.update_predictions(preds, mag_map_s.squeeze(2).permute(0,2,3,1))
