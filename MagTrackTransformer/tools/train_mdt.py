...
def train_epoch(
    train_loader, magnetic_noise, model, optimizer, train_meter, cur_epoch, cfg, writer=None
):
    """
    ...
    for cur_iter, (mag_map) in enumerate(train_loader):
        ...
        loss_fun = losses.get_loss_func(cfg.MODEL_MDT.LOSS_FUNC)(reduction="mean")

        # Add noise to the magnetic map.
        mag_map, gt_noise = misc.add_post_calib_noise(cfg, mag_map, magnetic_noise)
        preds = model(mag_map)
    
        #print(preds.shape, gt_noise.shape)
        loss = loss_fun(preds, gt_noise)
...
@torch.no_grad()
def eval_epoch(val_loader, magnetic_noise, model, val_meter, cur_epoch, cfg, writer=None):
    """
    ...
    for cur_iter, (mag_map) in enumerate(val_loader):
        ...
        val_meter.update_predictions(preds, gt_noise)
...
def train(cfg):
    ...
    # Print config.
    logger.info("Train MDT with config:")
    ...
    for cur_epoch in range(start_epoch, cfg.SOLVER.MAX_EPOCH):
        # Train for one epoch.
        train_epoch(
            train_loader, magnetic_noise, model, optimizer, train_meter, cur_epoch, cfg, writer
        )
        ...
        if is_eval_epoch:
            l1_err_epoch = eval_epoch(val_loader, magnetic_noise, model, val_meter, cur_epoch, cfg, writer)
