...
from utlis.meters import TrainMeter_orient as TrainMeter
from utlis.meters import ValMeter_orient as ValMeter
...
def train_epoch(
    ...
    for cur_iter, (mag_map_s, cam_pos, cam_ori) in enumerate(train_loader):
        ...
        loss_fun = losses.get_loss_func(cfg.MODEL_MOT.LOSS_FUNC)(reduction="mean")
...
def eval_epoch(val_loader, model, val_meter, cur_epoch, cfg, writer=None):
    ...
    for cur_iter, (mag_map_s, cam_pos, cam_ori) in enumerate(val_loader):
        ...
        val_meter.update_predictions(preds, cam_ori)
...
def train(cfg):
    ...
    # Print config.
    logger.info("Train MOT with config:")
    ...
    for cur_epoch in range(start_epoch, cfg.SOLVER.MAX_EPOCH):
        ...
        if is_eval_epoch:
            angular_err_epoch = eval_epoch(val_loader, model, val_meter, cur_epoch, cfg, writer)

            # Save the model with smallest val l1_err
            if angular_err_epoch < best_angular_err:
                best_angular_err = angular_err_epoch
