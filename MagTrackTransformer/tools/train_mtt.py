...
def train_epoch(
    ...
    for cur_iter, (mag_map_s, cam_data) in enumerate(train_loader):
        #print(mag_map_c.shape, mag_map_s.shape, cam_data.shape)
        ...
        preds = model(mag_map_s[:,3:,:,:,:] - mag_map_s[:,0:3,:,:,:])       #[bsz, 3]
...
def train(cfg):
    """
    Train an mtt for many epochs on train set and evaluate it on val set.
    Args:
        cfg (CfgNode): configs. 
    """ 
    ...
    # Print config.
    logger.info("Train Magnetic Tracking Transformer with config:")
